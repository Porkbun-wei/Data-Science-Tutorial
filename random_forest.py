...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        # take n random number from XX      
        # don't use random.sample, this is for without replacement
        # use random choice in the case you need replacement
        random_index = [random.choice(range(len(XX))) for _ in range (len(XX))]
        
        samples = [XX[i][0:(len(XX[0])-1)] for i in random_index ]
        
        labels = [XX[i][-1] for i in random_index ]  # class labels for the sampled records
        return (samples, labels)


    def bootstrapping(self, XX):
        # Initializing the bootstap datasets for each tree
        # this method changes the two lists initiated at the Randomforest class
        # the two list are bootstraps_datasets and bootstraps_labels
    ...
    def fitting(self):
        ...
        #num_tree mean number of tree you want to train
                
        
        for i in range(self.num_trees):
            X_sample =  self.bootstraps_datasets[i]
            y_sample =  self.bootstraps_labels[i]
            self.decision_trees[i].learn(X_sample,y_sample)
            #train multiple decision tree
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                # if the record is not in any out of bag sample, use every single tree
                # I developped to classify the record, then do a vote
                #print ("record not in bag")
                vote2 = []
                for i in range(self.num_trees):
                    decision = self.decision_trees[i].classify(record)
                    vote2.append(decision)
                
                counts2 = np.bincount(vote2)
...
def main():
    ...
    forest_size = 500  #Make take 5 min, should have around 84%
